...
class Building():
    def __init__(self, canvas, building_num, height, width):
        self.canvas = canvas
        self.height = height
        self.x = building_num * SPACING + 100
        self.width = width
    ...
    ''' is_inside tests if a bomb is inside the building '''

    def is_bomb_inside(self, point):
        print(point.X, self.x)
        if point.X < self.x - 5 or point.X > self.x + self.width or point.Y < CANVAS_HEIGHT - self.height:
            return False
        return True
...
class Display(Frame):
    def __init__(self, root):
        ...
        # create game objects
        self.plane = Plane(self.canvas, CANVAS_WIDTH + 100, 0)  # Plane initialised postion
    ...
    def check_bomb(self):
        if not self.bomb.falling:
            return
        # if not hit the building reset the status of bomb
        if self.bomb.position.getY() >= CANVAS_HEIGHT:
            self.bomb.miss()
            return
        # did the bomb hit a building?
        for building in self.buildings:
            if building.is_bomb_inside(self.bomb.position):
                self.bomb.explode()
                building.shrink()
...
class Game():
    ...
    def turn_pause(self):
        if self.pause == True:
            tkinter.messagebox.showinfo("Pause", "press enter to resume the game")
            self.pause = False
